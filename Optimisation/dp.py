...
vec_data = pd.read_csv('../DataSets/Data_LeaderBoard/InputDp.csv')
print vec_data.head()
...
for i in range(1,24*no_days):
	...
	for j in range(6):
		...
		if(j==5):
			...
			carry = math.ceil(demand[i]+5) - (demand[i])
			...
			carry = math.ceil(demand[i]+4) - (demand[i])
