...
# vec_data = pd.read_csv('../DataSets/InputDp.csv')		#### For Training
...
cost = [[sys.maxint for y in range(51)] for x in range(24*no_days)]
battery = [[sys.maxint for y in range(51)] for x in range(24*no_days)]
path = []
...
for i in range(24*no_days):
	...
	for j in range(51):
		exc_encounters = 0
		for k in range(2*(int(can_use)+1)):
			try:
				term = cost[i-1][j+k] + price[i]*(math.ceil(demand[i] - (k*0.8)/2))
				if(cost[i][j] >= term):
					carry = math.ceil(demand[i]-(k*0.8)/2) - (demand[i]-(k*0.8)/2)
					battery[i][j] = j + k 
			...
			try:
				term = cost[i-1][j-k] + price[i]*(math.ceil(demand[i] + k/2))
				if(cost[i][j] >= term):
					carry = math.ceil(demand[i]+k/2) - (demand[i]+k/2)
					battery[i][j] = j - k 
				cost[i][j] = min(cost[i][j], term)
			except:
				exc_encounters += 1
...
final_df.to_csv(output_dir + '23.csv', index=False, header=False) 
