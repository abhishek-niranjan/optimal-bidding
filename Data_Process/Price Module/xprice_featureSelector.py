...
# -*- coding: utf-8 -*-
"""
Created on Thu Jan  4 14:23:35 2018
...
train_file = '../../DataSets/Price_all_features_processed.csv'
...
def modelfit(alg, dtrain, predictors, dtest=X_test, useTrainCV=True, cv_folds=5,
    ...
    if useTrainCV:
        ...
        xgtrain = xgb.DMatrix(dtrain[predictors].values, label=dtrain['price_act'].values)
        cvresult = xgb.cv(xgb_param, xgtrain, num_boost_round=alg.get_params()['n_estimators'],
                          nfold=cv_folds, metrics='rmse', early_stopping_rounds=early_stopping_rounds)
        alg.set_params(n_estimators=cvresult.shape[0])

    #Fit the algorithm on the data
    alg.fit(dtrain[predictors], dtrain['price_act'], eval_metric='rmse')
    ...
    #scores
    rmse = sqrt(mean_squared_error(dtest['price_act'].values, dtest_predictions))
    mae = np.amax(np.fabs(dtest['price_act'].values - dtest_predictions))
    bid_loss_percentage = (np.sum(((dtest['price_act'].values - dtest_predictions) > 0)) / dtest.shape[0]) * 100
    r2, adj_r2 = adjr2(dtrain['price_act'].values, dtrain_predictions,
    ...
    print('Bid loss percentage = {:.5f}'.format(bid_loss_percentage))
...
def column_set_creator(n):
    column_set = []
    i = 0
    default = ['hour_of_day', 'price_td0', 'price_act']
    for j in range(n):
        column_set.append('price_t-' + str(i + 1))
        column_set.append('price_t+' + str(i + 1))
        column_set.append('price_d-' + str(i + 1))
        column_set.append('price_d+' + str(i + 1))
        column_set.append('solar_t-' + str(i + 1))
        column_set.append('solar_t+' + str(i + 1))
...
def fselection(lag):
    for i in range(1, lag + 1):
        ...
        predictors.remove('price_act')  # IMPORTANT!!
        ...
        recorder['bid_loss_percentage'].append(bid_loss_percentage)
...
df = pd.DataFrame(recorder,
                  columns=['lag_range', 'rmse', 'mae', 'adjusted_r2', 'r2', 'bid_loss_percentage'])
df.to_csv('price_feature_results.csv', index=False, sep=',')
